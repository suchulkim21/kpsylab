...
export function buildInterpretation(result: DarkNatureResult): DarkNatureInterpretation {
  ...
  if (veryHigh(traitScores.machiavellianism)) {
    badHighlights.push(
      '**냉혹한 체스 플레이어**: 타인을 감정을 가진 인격체가 아니라 도구로 보며, 자신의 이익을 위해 기꺼이 기만합니다.',
    );
    badRisks.push(
...
export function calculateDarkProfile(raw: DarkNatureResult): DarkProfile {
  ...
  // 개별 trait별 Good 텍스트 (high일 때) — 5배 확장
  const traitGoodTexts: string[] = [];
  if (high(m)) {
    traitGoodTexts.push(
      '**전략적 직관과 고도의 정치적 감각**이 돋보입니다. 복잡한 이해관계 속에서 누가 어떤 목표를 갖고 있는지, 권력이 어디로 흐르는지를 빠르게 읽어냅니다. ' +
      '장기 게임을 설계하는 능력이 뛰어나며, 단기적 감정에 휘둘리지 않고 목표 달성을 위한 최적 경로를 선택합니다. 협상·조직 정치·사업 개발에서 이 성향은 핵심 자산이 됩니다.'
    );
  }
  if (high(n)) {
    traitGoodTexts.push(
      '**무대 중앙에서 서사를 주도하는 카리스마와 비전**을 보유하고 있습니다. 사람들은 당신의 비전에 끌리고, 당신은 그 에너지를 하나의 방향으로 모아 큰 성과를 만들어낼 수 있습니다. ' +
      '자기 확신과 표현력이 뛰어나며, 위기 상황에서 팀의 상징적 중심이 되어 "이 사람을 따라가야 한다"는 신뢰를 이끌어냅니다. 리더십·브랜딩·캠페인·영향력 확대에 강점이 있습니다.'
    );
  }
  if (high(p)) {
    traitGoodTexts.push(
      '**극한 압박 상황에서도 감정적 동요 없이 목표를 달성하는 실행력**을 보여줍니다. 손실 공포나 불안에 휘둘리지 않고, 데이터와 시나리오에 기반한 결정을 내립니다. ' +
      '실패를 개인적 굴욕이 아니라 "한 판"으로 받아들이기 때문에 빠르게 다음 기회로 전환할 수 있으며, 위기·변동·불확실성이 큰 환경에서 냉정한 오퍼레이터로 기능합니다.'
    );
  }
  if (high(s)) {
    traitGoodTexts.push(
      '**경쟁 구도에서 주도권을 포기하지 않는 공격적이지만 전략적인 경쟁력**을 발휘합니다. 상대의 심리적 약점을 읽고, 필요한 순간에 단호하게 나서서 유리한 위치를 확보합니다. ' +
      '갈등을 두려워하지 않되, 감정적 폭발이 아니라 계산된 압박으로 사용할 수 있어, 협상·경쟁·주도권 싸움에서 강한 인상을 줍니다.'
    );
  }
  ...
  // 개별 trait별 Bad 텍스트 (high일 때) — 5배 확장
  const traitBadTexts: string[] = [];
  if (high(m)) {
    traitBadTexts.push(
      '당신에게 타인은 감정을 가진 존재가 아닌, **목적 달성을 위한 장기말**일 뿐입니다. 관계는 도구적 가치로만 평가되며, ' +
      '장기적 신뢰보다 즉각적 이익이 우선됩니다. 한 번 "쓸모없다"고 판단한 사람은 가차 없이 배제하며, 그 과정에서 죄책감을 거의 느끼지 않습니다. ' +
      '이 패턴이 반복되면 주변은 당신을 **위험한 협력자**로 인식하고, 진심과 정보를 숨깁니다. 결국 신뢰 자본이 바닥나 고립으로 이어질 수 있습니다.'
    );
  }
  if (high(n)) {
    traitBadTexts.push(
      '타인은 당신의 서사를 비추는 **거울 조각**일 뿐이며, 독립된 존재로 인정하지 않습니다. 누군가 당신과 다른 방식으로 빛나기 시작하면 그 사람의 자존감을 깎아내리려 하고, ' +
      '칭찬과 비난 모두 "내가 중심에 서 있는가"를 확인하는 도구가 됩니다. 비판을 받으면 문제의 핵심보다 "누가 나를 공격하는가"에 집중하며, 합리적 피드백조차 배신으로 받아들일 수 있습니다. ' +
      '장기적으로는 주변의 진심 어린 지지가 줄고, 아첨과 두려움만 남을 수 있습니다.'
    );
  }
  if (high(p)) {
    traitBadTexts.push(
      '타인의 고통이나 결과를 거의 고려하지 않고 **즉각적인 만족**을 추구하는 경향이 강합니다. 장기적 책임과 정서적 돌봄이 요구되는 관계가 부담스럽고, ' +
      '규칙과 윤리 경계를 "필요하면 넘을 수 있는 것"으로 보기 쉽습니다. 한순간의 분노·모멸감·스릴을 위해 돌이킬 수 없는 선택을 할 위험이 있으며, ' +
      '"그땐 어쩔 수 없었다"는 변명이 통하지 않는 순간 관계·커리어·법적 문제가 동시에 찾아올 수 있습니다.'
    );
  }
  if (high(s)) {
    traitBadTexts.push(
      '상대의 약점이나 불편함에서 **쾌감**을 느끼며, 갈등 상황을 필요 이상으로 끌고 갑니다. 타인의 고통을 관찰하고 유도하는 과정 자체가 에너지를 공급하며, ' +
      '상대가 예상한 만큼 흔들리지 않으면 통제감을 잃었다는 공포가 자신에게 되돌아옵니다. 이 패턴이 드러나면 주변은 당신을 **위험 인물**로 기억하고, ' +
      '신뢰와 협력을 거의 기대하지 않게 됩니다. 관계 파괴와 사회적 고립으로 이어질 수 있으므로, 상대의 고통을 "경계해야 할 영역"으로 인식하는 훈련이 필요합니다.'
    );
  }
  ...
  switch (archetype) {
    ...
    default:
      fullBadReport +=
        '당신의 Dark Nature는 단일한 악역 캐릭터로 정리되기보다는, 상황에 따라 다른 얼굴을 꺼내 쓰는 **멀티 아키타입 구조**에 가깝습니다. 이는 통제된 상태에서는 유연성으로 작동하지만, 스트레스가 누적되면 어떤 얼굴이 튀어나올지 스스로도 예측하기 어려워질 수 있습니다. 특정 트리거(배신감, 자존감 위협, 극한의 경쟁)에서 **전략적·공격적·조작적** 패턴이 활성화될 수 있으므로, 그 순간을 인지하고 선택을 늦추는 훈련이 중요합니다. 정기적으로 "지금 나는 어떤 모드인가?"를 점검하고, 경계선을 넘기 직전에 멈출 수 있는 **신호**를 스스로 정해 두는 것이 장기적인 관계와 자기 존중을 지킵니다.\n\n';
      break;
  }
...
export function assembleReport(
  ...
  // 전체 프로파일 요약 (5배 확장: 맥락·상황별·심화)
  if (totalDScore >= 80) {
    goodReport +=
      '이 프로파일은 고위험 감수 성향과 정교한 조종 능력이 결합된, 전형적인 하이-레벨 전략 인물의 패턴입니다. 당신은 단순히 판에 참여하는 플레이어가 아니라, 판의 구조와 규칙을 재설계하려는 쪽에 가깝습니다.\n\n' +
      '**맥락과 적용**: 조직의 최상위 의사결정, M&A·인수합병, 규제·리스크가 복잡한 사업, 정치·공공 정책 설계 같은 영역에서 당신의 프로파일은 **핵심 자산**으로 작용합니다. 상대방의 감정과 이해관계를 변수로 읽어내고, 장기 게임을 설계하는 능력은 소수의 엘리트만이 가진 재능입니다. 단, 이 강도가 지나치게 높으면 주변은 당신을 "예측 불가능한 위험 인자"로 인식할 수 있으므로, 공식·비공식 경계를 명확히 하고 **신뢰 자본**을 의도적으로 쌓는 습관이 중요합니다.\n\n' +
      '**상황별 해석**: 위기·구조조정·갈등 국면에서는 당신의 냉정함이 팀의 나침반이 됩니다. 반면, 일상의 협업·멘토링·감정적 지지가 필요한 순간에는 동료들이 "감정이 통하지 않는 사람"으로 느낄 수 있어, **선택적 개입**과 **역할 분리**가 효과적입니다. 당신이 전략 모드와 인간 모드를 구분할수록 장기적 영향력은 커집니다.\n\n' +
      '**심화**: 이 수준의 Dark Nature는 "선악"이 아니라 **에너지의 방향**으로 보는 것이 유용합니다. 당신의 두뇌는 효율·승리·구조 설계에 최적화되어 있으며, 그 에너지를 법과 윤리 안에서 쓰는지, 그 경계를 넘는지가 인생의 갈림길이 됩니다. 정기적으로 "내가 지금 누구의 신뢰를 쓰고 있는가", "이 선택이 5년 후 어떤 관계를 남기는가"를 점검하는 습관을 권합니다.';
  } else if (totalDScore >= 60) {
    goodReport +=
      '당신의 Dark Nature는 고위험·고수익 환경에서 활용 가능한 고급 전략 리소스로 작동합니다. 감정 소음을 최소화하고, 필요한 국면에만 선택적으로 개입하는 **전략적 오퍼레이터**에 가깝습니다.\n\n' +
      '**맥락과 적용**: 스타트업·투자·영업·협상·정치·복잡한 팀 리더십처럼 **변동성이 크고 보상이 불확실한** 영역에서 강점이 두드러집니다. 당신은 "안정보다 기회"를 선택할 수 있는 유연함을 갖추었으며, 상대의 감정을 읽되 그것에 휘둘리지 않는 **거리감**이 있습니다. 이 거리감이 지나치면 협력자들이 "차갑다"고 느낄 수 있으므로, 핵심 파트너에게는 의도적으로 **투명성**과 **일관된 태도**를 보여 주는 것이 장기 성과를 높입니다.\n\n' +
      '**상황별 해석**: 갈등·경쟁·협상 테이블에서는 당신의 판단력이 빛을 발합니다. 반면 팀이 "함께 위기를 넘기자"는 정서적 결속을 필요로 할 때는, 당신만 계산적이면 오해와 불신을 부를 수 있어, **한두 번은 감정적 지지**를 먼저 보여 준 뒤 전략을 꺼내는 순서가 효과적입니다. 당신의 Dark Nature는 "끄거나 켜는 스위치"가 아니라 **볼륨 조절**이 가능한 리소스로 다룰 때 가장 유용합니다.\n\n' +
      '**심화**: 이 강도는 "위험"과 "자산" 사이의 경계에 서 있습니다. 같은 성향이라도 **규칙과 신뢰를 지키는 쪽**으로 쓰이면 리더십과 협상력이 되고, **단기 이익과 조작**에 쓰이면 신뢰 붕괴와 고립으로 이어집니다. 주기적으로 "내가 지금 누구를 위한 게임을 하고 있는가"를 묻는 습관이 도움이 됩니다.';
  } else {
    goodReport +=
      '당신은 대부분의 상황에서 안정성과 협력을 우선하지만, 특정 임계점이 넘어가면 감정보다 계산을 우선하는 모드를 가동할 수 있는, 통제된 리스크 관리형 프로파일입니다.\n\n' +
      '**맥락과 적용**: 일상에서는 신뢰와 협력 기반의 관계를 중시하므로, 팀워크·장기 파트너십·고객 관계에서 **예측 가능하고 따뜻한** 이미지를 유지하기 쉽습니다. 다만 스트레스·배신감·극한의 경쟁 상황에서는 "평소와 다른" 전략적·방어적 모드가 활성화될 수 있어, **그런 순간을 인지하고 선택을 늦추는 것**만으로도 관계를 보호할 수 있습니다.\n\n' +
      '**상황별 해석**: 일반적인 업무·협업·일상적 갈등에서는 당신의 협력적 면이 주로 작동합니다. 반면, 권력 다툼·자원 쟁탈·심한 불공정이 느껴지는 상황에서는 내면의 **경계와 계산**이 올라올 수 있으므로, 그때의 선택이 "한 번의 예외"가 아니라 **습관이 되지 않도록** 의식하는 것이 좋습니다. 낮은 강도는 "없다"가 아니라 "잠재적·선택적"이라는 뜻입니다.\n\n' +
      '**심화**: 이 프로파일은 **균형**에 가깝습니다. Dark Nature를 "나쁜 것"으로만 보기보다, "특정 상황에서만 작동하는 도구"로 인정하면, 그 도구를 쓸 때와 내려놓을 때를 스스로 조절하기 쉬워집니다. 정기적으로 "지금 이 관계/이 결정에서 나는 협력 모드인가, 전략 모드인가?"를 점검하는 것만으로도 충분한 자기 관리가 됩니다.';
  }
  ...
  if (isPaid) {
    ...
    // SubFactor 강조 (도덕적 이탈·악의성 높을 때 해석 정확도 보강) — 5배 확장
    if (moralHigh) {
      fullBadReport +=
        '**도덕적 이탈** 수준이 높습니다. 결과가 좋다면 과정의 희생이나 기만을 정당화하는 경향이 있어, 장기적으로 신뢰 붕괴와 법적 리스크로 이어질 수 있습니다. ' +
        '이 수준이 지속되면 "목적이 수단을 정당화한다"는 믿음이 습관화되어, 처음에는 경미한 거짓말·정보 은폐에서 시작했다가 나중에는 **사기·배신·규칙 위반**까지 확대될 수 있습니다. ' +
        '주변은 당신의 "합리화"를 점점 믿지 않게 되고, 한 번이라도 큰 배신을 경험한 상대는 **영구적 불신**을 갖게 됩니다. 법·규제·내부 감사가 강한 환경에서는 한 번의 실수가 커리어와 인간관계를 동시에 무너뜨릴 수 있으므로, **과정의 투명성**을 의식적으로 높이는 것이 필수입니다.\n\n';
    }
    if (spiteHigh) {
      fullBadReport +=
        '**악의성** 수준이 눈에 띕니다. 타인의 불행이나 손해에서 쾌감을 느끼는 경향이 강화되면, 관계 파괴와 사회적 고립으로 이어질 수 있습니다. ' +
        '이 성향이 무의식적으로 드러나면 상대는 "이 사람은 내가 힘들 때 기뻐한다"고 느끼며, **진심 어린 협력**을 거의 기대하지 않게 됩니다. ' +
        'SNS·회의·일상 대화에서 상대를 깎아내리거나 약점을 건드리는 말이 반복되면, 피해자들은 당신을 **위험 인물**로 기억하고 네트워크에서 배제합니다. ' +
        '장기적으로는 "그 사람 때문에 자존감이 무너졌다"는 이야기가 퍼져, 새로운 기회와 인맥이 닫히고 **고립**만 남을 수 있습니다. 상대의 고통을 "경쟁의 결과"가 아닌 **인간으로서 경계해야 할 영역**으로 인식하는 훈련이 중요합니다.\n\n';
    }
    ...
    fullBadReport += '## D요인(일반적 어두운 성향) 해석\n\n';
    for (const { key, score } of dFactorMap) {
      ...
      if (text) fullBadReport += `**${labelKo}** (${levelKo}): ${text} ` +
        (level === 'High' ? '이 지표가 높을수록 관계·법·윤리 경계에서 리스크가 커질 수 있으므로, 의식적인 점검이 필요합니다.\n\n' : '현재 수준에서는 위험 신호로 보기 어렵지만, 스트레스나 극한 상황에서 변동할 수 있음을 참고하세요.\n\n');
    }

    // 리스크 심화 (5배 확장)
    fullBadReport += '## 리스크 심화 해석\n\n';
    fullBadReport +=
      '당신의 Dark Nature 프로파일은 **상황에 따라** 다른 얼굴을 보일 수 있습니다. 평소에는 통제된 전략가처럼 보이다가도, ' +
      '스트레스·위기·자존감 위협이 겹치는 순간 **충동적·공격적·조작적** 패턴이 튀어나올 수 있습니다. ' +
      '이 "전환점"을 스스로 인지하지 못하면, 한 번의 선택이 관계·커리어·법적 문제로 확대될 수 있으므로, **일상에서의 작은 신호**(예: 상대를 계속 깎아내리는 말, 목적을 위해 거짓을 정당화하는 생각)를 무시하지 마십시오.\n\n';
    fullBadReport +=
      '**관계 측면**: 신뢰는 한 번에 쌓이지 않고, 한 번 무너지면 복구가 거의 불가능합니다. 당신이 "전략"이라고 생각한 행동이 상대에게는 **배신**으로 각인될 수 있으므로, ' +
      '핵심 관계(파트너, 핵심 동료, 가족)에서는 "지금 내가 무엇을 숨기고 있는가", "이 선택이 상대에게 어떻게 보일까"를 꼭 점검하십시오. ' +
      '장기적으로는 **진심을 보여 준 사람**만이 위기 때 당신 곁에 남으며, 나머지는 정보와 협력을 멀리합니다.\n\n';
    fullBadReport +=
      '**법·윤리 측면**: "합법적"인 선과 "도덕적"인 선은 다릅니다. 법의 테두리 안에서도 불공정 거래·정보 비대칭·정서적 착취는 **평판과 신뢰**를 파괴하고, ' +
      '한 번 퍼진 평판은 수년간 따라다닙니다. 특히 권력·정보·자금이 있는 위치일수록, 한 번의 경계선 침범이 **집단적 반발**과 제재로 이어질 수 있으므로, ' +
      '의사결정 전에 "이것이 공개되면 나는 어떻게 설명할 수 있을까?"를 스스로에게 묻는 습관을 권합니다.\n\n';
    fullBadReport +=
      '**자기 관리**: 이 리포트는 당신을 "나쁜 사람"으로 규정하기 위한 것이 아닙니다. Dark Nature는 **에너지의 방향**이며, 그 방향을 인식하고 ' +
      '"언제 쓸 것인가, 언제 멈출 것인가"를 선택하는 것이 성인으로서의 역량입니다. 정기적으로 신뢰하는 사람(상담자, 멘토, 파트너)과 이 프로파일에 대해 이야기하고, ' +
      '**경계선을 넘기 직전의 순간**을 되돌아보는 것이 장기적인 관계와 자기 존중을 지키는 데 도움이 됩니다.\n\n';
