...
export function getConflictInsight(input: ConflictInsightInput): string | null {
  ...
  const tensionPhrase =
    "내면의 본성과 지향하는 이상향 사이의 역동적인 긴장이 감지되었습니다. 이것이 당신의 성장을 견인하는 핵심 동력입니다. ";

  const conflicts: string[] = [];

  if (m1Type === "A" && !highP) {
    conflicts.push(
      `${tensionPhrase}무의식 방해 요인(M1)에서는 ${m1Label} 성향이 두드러지나, 시나리오 테스트(M2)에서는 주도적 행동보다 신중한 관망을 선택하는 경향이 있습니다. ${hint.social}에서는 완벽을 추구하다가도, ${hint.personal}에서는 실수를 피하려는 심리적 균형을 시사합니다.`
    );
  }
  if (m1Type === "B" && highSd) {
    conflicts.push(
      `${tensionPhrase}M1에서는 타인의 인정에 대한 강한 갈망이 관찰되나, M2에서는 사회적 거리두기·독립적 선택을 선호합니다. 겉으로는 거리를 두는 듯하나, 내면에서는 인정받지 못할까 봐 조심하는 '역설적 자기보호'가 작동하고 있을 수 있습니다.`
    );
  }
  if (m1Type === "C" && highP) {
    conflicts.push(
      `${tensionPhrase}감정 회피 성향(M1)이 강한 반면, 시나리오에서는 적극적·직접적 대응을 선택했습니다. ${hint.social}에서는 행동으로 표출하되, ${hint.personal}에서는 감정을 억제하는 상황별 페르소나 분리가 나타납니다.`
    );
  }
  if (m1Type === "D" && highA && !highP) {
    conflicts.push(
      `${tensionPhrase}M1에서 충동·회피 성향이 있었으나, M2에서는 유연한 적응과 신중한 대안 탐색을 보였습니다. 외부 자극이 많은 상황에서는 속도감을 추구하다가도, 구조화된 시나리오에서는 더 신중한 판단을 내리는 심층 심리의 변화 과정을 보여줍니다.`
    );
  }
