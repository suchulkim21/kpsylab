...
export function calculateIntermediateResult(phase: number, options: ScenarioOption[]) {
    ...
    if (phase === 3) {
        if (proactivity >= adaptability && socialDistance >= adaptability) {
            ...
            neurologicalInsights = `시스템 관점에서, 체제 구축가 유형은 전두엽(특히 배외측전두피질)의 구조화와 계획 기능이 발달해 있습니다. 세로토닌 시스템이 안정적이어서 규칙과 일관성에서 편안함을 느끼며, 코르티솔 반응이 예측 가능한 스트레스에서는 오히려 동기 부여로 작용합니다. 하지만 예상치 못한 변화에서는 아미그달라(공포/불안 중추)가 과도하게 반응할 수 있어, 변화 관리 능력을 키우는 것이 중요합니다.`;
            ...
            neurologicalInsights = `시스템 관점에서 도파민 D4 수용체 변이와 연관이 있으며, 노르에피네프린(각성 신경전달물질) 수치가 높아 각성 상태를 유지할 수 있습니다. 전두엽의 실행 기능보다는 창의적 사고를 담당하는 영역이 활발하며, 디폴트 모드 네트워크(DMN)가 활성화되어 있어 아이디어 발상이 용이합니다. 하지만 주의 집중력이 다중 작업에 분산될 수 있어, 중요한 일에는 딥 워크 환경이 필요합니다.`;
            ...
            neurologicalInsights = `시스템 관점에서 복내측 전전두피질(vmPFC)의 사회적 판단 기능과 섬엽(insula)의 감정 인식 기능이 발달해 있습니다. 옥시토신 수용체 유전자 변이와 연관이 있을 수 있으며, 세로토닌 시스템이 안정적이어서 사회적 상호작용에서 편안함을 느낍니다. 하지만 과도한 사회적 기대에 대한 스트레스에서 코르티솔이 증가할 수 있어, 자기 보호 전략이 필요합니다. 디폴트 모드 네트워크에서 타인의 관점을 시뮬레이션하는 능력이 뛰어나지만, 자기 반성 시간도 확보해야 합니다.`;
            ...
            neurologicalInsights = `시스템 관점에서 전전두피질의 자기 인식과 자기조절 기능이 발달해 있습니다. 도파민 보상 시스템이 내적 목표 달성에서 활성화되며, 외부 평가보다 자기 평가에서 동기를 얻습니다. 자율신경계가 독립적인 환경에서 더 안정적으로 작동하며, 사회적 상호작용 시에는 코르티솔이 증가할 수 있습니다. 집중 시에는 감마파(깊은 집중)와 알파파(창의적 사고)가 활성화되어 깊이 있는 작업을 수행할 수 있습니다.`;
        ...
        if (maxScore === proactivity) {
            ...
            neurologicalInsights = `시스템 관점에서 전전두엽의 실행 기능과 운동 피질의 활동이 높습니다. 도파민 보상 경로가 목표 달성과 행동 실행에서 활성화되며, 노르에피네프린(각성 물질) 수치가 높아 빠른 반응이 가능합니다. 하지만 과도한 스트레스 상황에서는 감정 조절 영역(전대상피질)의 활동을 의식적으로 높이는 것이 필요합니다.`;
            ...
            detailedAnalysis = `적응력 점수 ${aPercent}% (${aPercentile})로 가장 높게 나타났습니다. 외부 자극이나 환경 변화에 유연하게 대응하며, 고정된 패턴보다는 상황에 맞는 최적의 대응 방식을 선택합니다. 변화를 두려워하지 않고 오히려 새로운 상황을 기회로 삼는 성향이 있습니다. 시스템 관점에서 전전두엽의 인지 유연성과 변환 기능이 발달해 있어, 상황 변화에 빠르게 재구성할 수 있는 능력이 뛰어납니다.`;
